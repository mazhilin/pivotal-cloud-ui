...
Vue.directive('drag', function(el, binding) {
  ...
  window.vm.$nextTick(() => {
    ...
    const setPoint = () => {
      setParentNode()
      const w_w = parameter.parentNodeInfo.width - parameter.currentNodeInfo.width
      if (parameter.currentNodeInfo.translateX >= w_w) { // 是否溢出右边
        parameter.currentNodeInfo.translateX = w_w
      }
      const h_h = parameter.parentNodeInfo.height - parameter.currentNodeInfo.height
      if (parameter.currentNodeInfo.translateY >= h_h) { // 是否溢出底部
        parameter.currentNodeInfo.translateY = h_h
      }
      // 设置最小的宽高
    ...
    const changeSize = (e, $position = 'right') => {
      ...
      document.onmousemove = (e) => {
        ...
        // 用鼠标的位置减去鼠标相对元素的位置，得到元素的位置
        const chance_W = e.clientX - dragChildDisX
        const chance_H = e.clientY - dragChildDisY
        switch ($position) {
          case 'top': // 顶部 ok
            parameter.currentNodeInfo.height = firstHeight - chance_H
            parameter.currentNodeInfo.translateY = firstTranslateY + chance_H
            break
          case 'bottom':// 底部 ok
            parameter.currentNodeInfo.height = firstHeight + chance_H
            break
          case 'right': // 右边 ok
            parameter.currentNodeInfo.width = firstWidth + chance_W
            break
          case 'left':// 左边 ok
            parameter.currentNodeInfo.width = firstWidth - chance_W
            parameter.currentNodeInfo.translateX = firstTranslateX + chance_W
            break
          case 'bottomLeft':// 左下角
            parameter.currentNodeInfo.width = firstWidth - chance_W
            parameter.currentNodeInfo.height = firstHeight + chance_H
